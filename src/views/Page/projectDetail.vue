<template>
  <div class="project-detail">
    ...
    <div class="detail-info">
      <h2 class="detail-info-title"><i class="iconfont iconjianjie"></i>基本信息</h2>
      ...
      <h2 class="detail-info-title"><i class="iconfont iconjianjie1"></i>项目简介</h2>
      <p class="detail-desc">{{ xiangmu_desc }}</p>
    </div>
    <div class="credit-info">
       <h2 class="detail-info-title"><i class="iconfont iconzuanshi"></i>信用状况</h2>
      <ul class="credit-list">
        ...
        <li>
          ...
          <p>
            <span v-if="is_dishonesty === 0">机构或机构负责人未存在失信行为</span>
            <span v-else>机构或机构负责人存在失信行为</span>
            <br>
            <span v-if="is_punish === 0">机构未受到登记管理机关或其他部门行政处罚</span>
            <span v-else>机构受到登记管理机关或其他部门行政处罚或行政处罚尚未执行完毕</span>
            <br>
            <span v-if="is_investigation === 0">机构或机构负责人未被司法机关或有关部门立案调查</span>
            <span v-else>机构或机构负责人被司法机关或有关部门立案调查</span>
             <br>
            {{ level }}级社会组织（有效期2017年--2019年）<br>
            运作经验：{{experience}}
          </p>
        </li>
      </ul>
    ...
    <div class="opinion-info" v-if="instructions">
       <h2 class="detail-info-title"><i class="iconfont iconyijian"></i>项目推介意见 </h2>
      <p>{{instructions}}</p>
    </div>
    <div class="support-info">
       <h2 class="detail-info-title"><i class="iconfont iconxinyong"></i>资助信息</h2>
      <ul class="detail-info-list">
        <li>
    ...
    <div class="about-box">
      ...
      <div class="about-list">
        <router-link class="about-item" :to="'/Content/projectDetail?id=' + item.id" v-for="(item,index) in relevant" :key="index">
          <div>
...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      this.xiangmu_desc = _data.data.xiangmu_desc
...
.project-detail {
  ...
  .detail-info {
    ...
    .detail-info-list {
      ...
      li {
        ...
        min-height: 60px;
        padding: 15px 0;
        border-bottom: 1px solid rgba(255, 177, 168, .2);
        color: #fff;
        font-size: 26px;
  ...
  .credit-info {
    padding: 15px;
    margin-bottom: 20px;
    background-image: linear-gradient(0deg, #ffb9b9 0%, #fae6e6 100%), linear-gradient(#e6f2fa, #e6f2fa);
    .detail-info-title {
      display: flex;
      align-items: center;
      height: 60px;
